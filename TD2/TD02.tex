...
\begin{solution}
$36_{10} = 00100110_{2}$ dans les 3 \'ecritures.
...
Quelle seraient ces valeurs en compl\'ement \`a 1 ? En signe plus valeur absolue ?
\begin{solution}
...
$11110110_{2} + 10100110_{2} = 10011100_{2}$

$00101101_{2} + 01101111_{2} = 10011011_{2}$
...
\item Deux méthodes existent pour convertir la représentation complément à 2 en
  décimal:
\begin{itemize}
  \item soustraire 1 et faire un NON logique bit à bit (ce qui revient à passer par la
    représentation en complément à 1) lorsque le bit de poids fort est à 1.
  \item utiliser la décomposition suivante :
    $$ (a_{n-1}a_{n-2}\dots a_{0})_{\textrm{cpl2}} = -a_{n-1}.2^{n-1} + a_{n-2}.2^{n-2} + \dots + a_0.2^{0}$$
\end{itemize}
Montrer que les deux méthodes sont équivalentes.
...
2^n - x =& 1 + \overline{x}\\
\overline{x} =& 2^n - x - 1 \quad (1)\\
\end{align*}

Soit $p = (1p_{n-2}\dots p_{0})_{CA2} <0$
On veut montrer que $ - \overline{p-1} = -2^{n-1} + p_{n-2}2^{n-2} + \dots +
p_{0}2^0$

En utilisant $(1)$,
\begin{align*}
  \overline{p-1} =& 2^n - (p_{bin}-1) -1\\
  \overline{p-1} =& 2.2^{n-1} - p_{bin}\\
  \overline{p-1} =& 2.2^{n-1} - (2^{n-1}+p_{n-2}2^{n-2}+ \dots + p_{0}2^0)\\
  \overline{p-1} =& 2^{n-1} -p_{n-2}2^{n-2}- \dots - p_{0}2^0\\
  - \overline{p-1} =& -2^{n-1} +p_{n-2}2^{n-2}+ \dots + p_{0}2^0 \quad (cqfd)\\
\end{align*}
...
Nous souhaitons écrire une fonction \texttt{char get\_size(u\_int32\_t M);} qui
...
\begin{solution}
\begin{verbatim}
char get_size_faster(u_int32_t M) {
    char count = 0;
    while(M) {
