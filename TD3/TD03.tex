...
\begin{enumerate}
  \item Écrire une fonction C qui retourne la position du bit à 1
    le plus à gauche dans un mot de 32 bits. Utiliser une boucle
    et l'opérateur \verb!>>!. Quel est le nombre d'itérations
    maximum effectués ?
    \begin{solution}
\begin{verbatim}
...
\item La distance de Hamming $d(M,N)$ entre deux mots $M$ et $N$ représente le
  nombre de bits différents entre les mots . Ainsi, $d(1001, 1010) = 2$.
  Calculer $d(C_6, C_9)$ ?
  Calculer $d(C_5, C_6)$ ?
  \begin{solution}
    $d(C_6, C_9) = 2$
...
\item La distance de Hamming $d_C$ d'un code est definie comme la plus petite
  ...
  \begin{solution}
    ...
    Deuxième cas: $n_i = n_j$ et $m_i \neq m_j$ (ou vice-versa), la distance
    de hamming est 2, il faut pour passer de $C_i$ à $C_j$ éteindre $m_i$ et
    allumer $m_j$.
...
\item La distance de Hamming d'un code permet de calculer combien d'erreurs
  peuvent être detectés $e_d$ et combien d'erreurs peuvent être corrigées $e_c$.
  Ainsi, $e_d = d_C - 1$ et $e_c = \lfloor \frac{d_C-1}{2} \rfloor$.
  Combien d'erreurs peuvent être détectées par le code 2/5 ?
  Combien d'erreurs peuvent être corrigés ?
...
\begin{solution}
  \begin{itemize}
  \item Prennons deux mots consécutifs du code: $00000$ et $10001$,
...
\item Combien d'erreurs peuvent être détectées ? Combien peuvent être
    corrigées ?
\begin{solution}
  On peut détecter $2-1=1$ erreurs et on peut en corriger aucune.
\end{solution}
\end{enumerate}
...
\begin{enumerate}
  \item Calculer la distance de Hamming du code à répétition 3.
    ...
    \end{solution}
  \item Combien d'erreurs peuvent être detectées ? Combien peuvent être
    corrigées ?
    \begin{solution}
  ...
  \item Vous recevez les messages suivants: indiquez si des erreurs se
    sont produites, le cas échéant corrigez les.
  ...
  \item Vous disposez d'un canal de communications dont la probabilité
    ...
    distincts la probabilité d'avoir une erreur sur le bit $n$ et une erreur sur
    ...
    \begin{solution}
        ...
        $N_n$ suit une loi binomiale (car les erreurs sont des évenements
      ...
      \begin{itemize}
        ...
        \item Soit $P_1$ a probabilité d'avoir une ou zéro erreurs par bloc de trois bits:
...
\begin{enumerate}
  \item En s'appuyant sur le diagramme de Venn, si on change un bit des données
  ...
  \item Quelle est donc la distance de Hamming du code ? Combien d'erreurs
    peuvent être detectées ? Combien peuvent être corrigées ?
    \begin{solution}
  ...
  \item Vous recevez les messages suivants, indiquez si une erreur s'est
    produite, le cas écheant corrigez le mot reçu: $ 0001111 \quad 1110010 \quad
